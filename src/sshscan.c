...
void* ssh_test(void *arg)
{
    ...
    pthread_mutex_lock(&(test_arg->setting->setting_mutex));/*加锁*/
    
    record_node->next = test_arg->setting->workarg_list_head;
    test_arg->setting->workarg_list_head = record_node;

    pthread_mutex_unlock(&(test_arg->setting->setting_mutex));/*解锁*/
...
void *try_login_user(void *arg)
{
    ...
    pthread_mutex_lock(&(parg->setting->setting_mutex));/*加锁*/
    
    record_node->next = parg->setting->workarg_list_head;
    parg->setting->workarg_list_head = record_node;

    pthread_mutex_unlock(&(parg->setting->setting_mutex));/*解锁*/
...
int test_connect(char *ip, short port, int test_count)
{
    ...
    timeo.tv_sec = 30;  /*超时时间为30秒,内核默认时间75秒或120秒*/
...
int checkSetting(int argc, char **argv, Setting *setting)
{
    ...
    setting->port = 22;     /*默认22端口，如果是*/
    setting->workarg_list_head = NULL;
    setting->connect_test_count = 3;  /*连接测试次数*/
    setting->pwd_group_num = 4; /*把密码列表4组*/
    setting->pwd_groups = NULL;  /*分组密码数组*/
    setting->per_pwd_num = 0;     /*每组大小*/
    setting->thread_num = 6;  /*线程数*/
    
    home_path = getenv("HOME");  /*初始化日志目录/$HOME*/
    if (home_path == NULL)
    {
